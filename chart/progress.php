...
SELECT alr_parcel_query.prov_code,alr_parcel_query.prov_nam_t as name,ss.sum
FROM alr_parcel_query
left join ss ON alr_parcel_query.prov_code = ss.prov_code
group by alr_parcel_query.prov_code,alr_parcel_query.prov_nam_t,ss.sum ;");
			}
			else if ($prov_name != '0' and $amphoe_name == '0'){
			 $result1 = pg_query( "with ss as (
...
SELECT alr_parcel_query.amp_nam_t  as name,alr_parcel_query.amp_code,ss.sum
FROM alr_parcel_query
left join ss ON alr_parcel_query.amp_code = ss.amp_code
where prov_code = '$prov_name'
group by alr_parcel_query.amp_nam_t,alr_parcel_query.amp_code,ss.sum;");
			}
			else if ($prov_name != '0' and $amphoe_name != '0' and $tambon_name == '0') {  
			 $result1 = pg_query( "with ss as (
...
SELECT alr_parcel_query.tam_nam_t as name,alr_parcel_query.tam_code,ss.sum
FROM alr_parcel_query
left join ss ON alr_parcel_query.tam_code = ss.tam_code
where prov_code = '$prov_name' and amp_code = '$amphoe_name'
group by alr_parcel_query.tam_nam_t,alr_parcel_query.tam_code,ss.sum;");
			}
			else if ($prov_name != '0' and $amphoe_name != '0' and $tambon_name != '0') {
			 $result1 = pg_query( "with ss as (
SELECT tam_code,tam_nam_t   , count(*) as sum
...
FROM alr_parcel_query
left join ss ON alr_parcel_query.tam_code = ss.tam_code
where prov_code = '$prov_name' and amp_code = '$amphoe_name' and alr_parcel_query.tam_code = '$tambon_name'
group by alr_parcel_query.tam_nam_t,alr_parcel_query.tam_code,ss.sum;");
			}
