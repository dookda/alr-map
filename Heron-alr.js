            ...
            if (action === Ext.data.Api.actions.read) {
                ...
                Ext.each(records, function(r, i) {
                    ...
                    feature.modified = Ext.apply(feature.modified || {}, {
                        attributes: Ext.apply((feature.modified && feature.modified.attributes) || {}, r.modified)
                    });
                }, this);
                var o = {
                    action: action,
    ...
    readers: {
        "tms": {
            ...
            "BoundingBox": function(node, obj) {
                obj.bbox = new OpenLayers.Bounds(node.getAttribute("minx"), node.getAttribute("miny"), node.getAttribute("maxx"), node.getAttribute("maxy"));
            },
            "Origin": function(node, obj) {
                obj.origin = new OpenLayers.LonLat(node.getAttribute("x"), node.getAttribute("y"));
            },
...
OpenLayers.Format.CSWGetRecords.v2_0_2.prototype.write = function(options) {
    ...
    this.setAttributeNS(node, this.namespaces.xmlns, "xmlns:gmd", this.namespaces.gmd);
...
OpenLayers.Format.SOSGetObservation.prototype.write = function(options) {
    ...
    this.setAttributeNS(node, this.namespaces.xmlns, "xmlns:om", this.namespaces.om);
    this.setAttributeNS(node, this.namespaces.xmlns, "xmlns:ogc", this.namespaces.ogc);
...
OpenLayers.Format.WFST.v1.prototype.writers = {
    "wfs": {
        ...
        "Update": function(obj) {
            ...
            if (this.featureNS) {
                this.setAttributeNS(node, this.namespaces.xmlns, "xmlns:" + this.featurePrefix, this.featureNS);
            }
            ...
            for (var key in feature.attributes) {
                if (feature.attributes[key] !== undefined && (!modified || !modified.attributes || (modified.attributes && (key in modified.attributes)))) {
                    this.writeNode("Property", {
                        name: key,
...
Ext.extend(GeoExt.data.WFSCapabilitiesReader, Ext.data.DataReader, {
    ...
    readRecords: function(data) {
        ...
        var url = (parseFloat(data.version) >= 1.1) ? data.operationsMetadata && data.operationsMetadata["GetFeature"].dcp.http.post[0].url : data.capability.request.getfeature.href.post;
...
(function() {
    ...
    GeoExt.data.AttributeReader && Ext.intercept(GeoExt.data.AttributeReader.prototype, "readRecords", keepRawWFS);
})();
GeoExt.data.CSWRecordsReader = function(meta, recordType) {
    ...
    GeoExt.data.CSWRecordsReader.superclass.constructor.call(this, meta, recordType);
};
Ext.extend(GeoExt.data.CSWRecordsReader, Ext.data.JsonReader, {
    ...
    readRecords: function(data) {
        ...
        var result = GeoExt.data.CSWRecordsReader.superclass.readRecords.call(this, data);
        ...
        create: function() {
            ...
            if (Heron.app) {
                ...
                if (Heron.layout.mapContextUrl) {
                    var mgr = Heron.App.contextManager = new Heron.data.HeronMapContext({
                        name: Heron.layout.mapContextUrl,
                        options: Heron.layout.mapContextOptions
                    });
...
Heron.data.MapContext = {
    ...
    saveContext: function(mapPanel, options) {
        ...
        var formFields = [{
            ...
            type: 'hidden',
            name: 'filename',
            value: options.fileName + options.fileExt
        }, {
            tag: 'input',
            type: 'hidden',
            name: 'mime',
...
Heron.data.MapContextParser = {
    ...
    parse: function(xml, loader) {
        ...
        this.context.map.layers = this.context.map.layers.concat(this.parseTMS(xml), this.parseImage(xml), this.parseWMTS(xml), this.parseWMS(xml), this.parseWFS(xml, this.context.map.settings.projection), this.parseAtom(xml), this.includedLayers);
        return this.context;
    },
    blankLayer: function() {
        var result = [];
        var blank = new OpenLayers.Layer.Image("Blank", "data:image/gif;base64,R0lGODlhAQABAID/AMDAwAAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==", OpenLayers.Bounds.fromString(this.context.map.settings.maxExtent), new OpenLayers.Size(10, 10), {
    ...
    parseImage: function(xml) {
        ...
        for (var i = nodes.length - 1; i >= 0; i--) {
            ...
            var imageLayer = new OpenLayers.Layer.Image(title, url, OpenLayers.Bounds.fromString(this.context.map.settings.maxExtent), new OpenLayers.Size(size[0], size[1]), config);
    ...
    parseWMTS: function(xml) {
        ...
        for (var i = nodes.length - 1; i >= 0; i--) {
            ...
            var featureInfoFormat = this.getTextContent('featureInfoFormat', nodes[i]);
            var config = {
                name: title,
                url: url,
                isBaseLayer: isBaseLayer,
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if (node.tagName == 'contextFolder') {
                ...
                if (url.length) {
                    ...
                    this.includedLayers = this.includedLayers.concat(this.parseImage(result), this.parseTMS(result), this.parseWMTS(result), this.parseWMS(result), this.parseWFS(result, this.context.map.settings.projection), this.parseAtom(result));
    ...
    getLayerUrlContent: function(elementName, node) {
        ...
        if (result && result.indexOf("{baseUrl}") > -1) {
            ...
            if (baseUrl) {
                ...
                result = result.replace("{baseUrl}", this.context.map.settings.baseUrl);
            } else {
    ...
    getStyleContent: function(elementName, root) {
        ...
        if (node) {
            ...
            for (var i = 0; i < styleNodes.length; i++) {
                ...
                var propertyNodes = Ext.DomQuery.jsSelect('property', styleNodes[i]);
                var styleOptions = {};
                for (var j = 0; j < propertyNodes.length; j++) {
                    var name = this.getTextContent('name', propertyNodes[j]);
                    var value = this.getTextOrNumberContent('value', propertyNodes[j]);
...
Ext.ux.SmallSpinner = Ext.extend(Ext.util.Observable, {
    ...
    spin: function(down, alternate) {
        ...
        if (v < 10) {
            v = "0" + v;
            if (this.field.validator(v)) {
                this.field.setValue(v);
            }
        } else this.field.setValue(v);
    },
...
Ext.ux.form.SmallSpinnerField = Ext.extend(Ext.form.TextField, {
    ...
    constructor: function(config) {
        ...
        Ext.ux.form.SmallSpinnerField.superclass.constructor.call(this, Ext.apply(config, {
            plugins: plugins
        }));
    },
...
Heron.widgets.LayerNodeMenuItem.Style = Ext.extend(Heron.widgets.LayerNodeMenuItem, {
    ...
    handler: function(menuItem, event) {
        ...
        new Ext.Window({
            ...
            items: [gxp.VectorStylesDialog.createVectorStylerConfig(layerRecord)]
        }).show();
    },
    isApplicable: function(node) {
...
Heron.widgets.LayerNodeMenuItem.LayerInfo = Ext.extend(Heron.widgets.LayerNodeMenuItem, {
    ...
    handler: function(menuItem, event) {
        ...
        Ext.MessageBox.show({
            ...
            msg: String.format('Placeholder: should become more extensive with infos, metadata, etc.!<br>' + "<br>Name: {0}" + "<br>Type: {1}" + "<br>Tiled: {2}" + "<br>Has feature info: {3}" + "<br>Has WFS: {4}", layer.name, layerType, tiled, hasFeatureInfo, hasWFS),
            buttons: Ext.Msg.OK,
            fn: function(btn) {
                if (btn == 'ok') {}
            },
...
Heron.widgets.ActiveLayersPanel = Ext.extend(Ext.tree.TreePanel, {
    ...
    initComponent: function() {
        ...
        if (this.contextMenu) {
            var cmArgs = this.contextMenu instanceof Array ? {
...
Ext.extend(GeoExt.tree.CapabilitiesViewLoader, Ext.tree.TreeLoader, {
    ...
    processLayer: function(XmlEl, prevLink) {
        ...
        if (XmlEl.tagName == "inspire_common:URL" || XmlEl.tagName == "xlink:href" || XmlEl.tagName == "href") {
            link = true;
        }
        ...
        Ext.each(XmlEl.attributes, function(a) {
            ...
            if (a.nodeName == "inspire_common:URL" || a.nodeName == "xlink:href" || a.nodeName == "href") {
...
Heron.widgets.CapabilitiesViewPanel = Ext.extend(Ext.tree.TreePanel, {
    ...
    onAfterRender: function() {
        ...
        this.updateViewTree();
        map.events.register("changelayer", this, this.updateViewTree);
    },
    ...
    addLayers2Tree: function() {
        ...
        if (map !== undefined) {
            for (var i = 0; i < map.layers.length; i++) {
                ...
                if (!(layer instanceof OpenLayers.Layer.WMS) || this.addedUrl.indexOf(layer.url) != -1) {
                    continue;
                }
...
Heron.widgets.search.CoordSearchPanel = Ext.extend(Ext.form.FormPanel, {
    ...
    initComponent: function() {
        ...
        this.storeZoom = new GeoExt.data.ScaleStore({
            map: map
        });
        ...
        this.items = [{
            ...
            padding: 5,
            items: [self.tLabel, self.pCombo, self.xField, self.yField, self.sCombo, self.mCheckbox, self.cCheckbox, self.rLabel],
...
OpenLayers.Protocol.NGSI10 = OpenLayers.Class(OpenLayers.Protocol.HTTP, {
    ...
    parseFeatures: function(request) {
        ...
        if (this.refreshMillis > 0) {
            this.timeoutId = setTimeout(OpenLayers.Function.bind(this.onRefresh, this), this.refreshMillis);
        }
        return features;
    },
...
GeoExt.data.LayerRecord.prototype.fields.addAll([new Ext.data.Field({
    name: "group",
    type: "string",
    mapping: "group"
}), new Ext.data.Field({
...
Heron.widgets.GXP_LayerPanel = Ext.extend(Ext.Panel, {
    ...
    initTools: function() {
        if (this.tools && this.tools.length > 0) {
            ...
            for (var i = 0, len = this.tools.length; i < len; i++) {
                try {
                    tool = Ext.ComponentMgr.createPlugin(this.tools[i], this.defaultToolType);
                } catch (err) {
    ...
    addLayerSource: function(options) {
        ...
        config.id = id;
        try {
            source = Ext.ComponentMgr.createPlugin(config, this.defaultSourceType);
        } catch (err) {
...
Heron.widgets.LayerTreePanel = Ext.extend(Ext.tree.TreePanel, {
    ...
    initComponent: function() {
        ...
        var options = {
            ...
            listeners: {
                ...
                movenode: function(tree, node, oldParent, newParent, index) {
                    ...
                    if ((this.blnCustomLayerTree == true) && (this.ordering == 'TopBottom')) {
        ...
        Ext.apply(this, options);
        this.addEvents("beforelayerselectionchange", "layerselectionchange");
    ...
    createNode: function(treeLoader, attr) {
        ...
        var layerRecord = this.getLayerRecord(attr.layer),
            layer;
        if (layerRecord) {
        ...
        if (layer) {
            ...
            if (this.layerIcons == 'bylayertype' && !(attr.iconCls || attr.cls || attr.icon)) {
                ...
                if (layerType == 'Vector') {
                    layerKind = 'vector';
                    if (layer.protocol && layer.protocol.format && layer.protocol.format.CLASS_NAME && layer.protocol.format.CLASS_NAME.indexOf('Atom') > -1) {
                        layerKind = 'atom';
                    }
                }
...
Heron.widgets.LayerLegendPanel = Ext.extend(GeoExt.LegendPanel, {
    ...
    getLegendUrlFromCapabilities: function(serviceUrl, layerName, styleName) {
        var layerCaps = Heron.data.OWSCapabilitiesCache.getInstance().getWMSLayerCapability(serviceUrl, layerName, {
            async: false
        });
        if (!layerCaps) {
...
Heron.widgets.GoogleStreetViewPanel = Ext.extend(Ext.Panel, {
    ...
    afterRender: function() {
                ...
                if (GeoExt.Popup) {
                    this.bubble(function(cmp) {
                        if (cmp instanceof GeoExt.Popup) {
                            this.location = cmp.location.clone().transform(cmp.map.getProjectionObject(), new OpenLayers.Projection("EPSG:4326"));
                            return false;
                        }
                    }, this);
                }
        ...
        var panorama = this.panorama = new google.maps.StreetViewPanorama(this.body.dom, options);
...
Heron.widgets.MapPanel = Ext.extend(GeoExt.MapPanel, {
    ...
    initComponent: function() {
        ...
        if (gxMapPanelOptions.map.allOverlays && gxMapPanelOptions.map.layers && (gxMapPanelOptions.map.layers[0].minResolution || gxMapPanelOptions.map.layers[0].maxResolution)) {
            var j = 0,
                layers = gxMapPanelOptions.map.layers;
...
Heron.widgets.SimpleTimeSliderPanel = Ext.extend(Ext.Panel, {
    ...
    initComponent: function() {
                    ...
                    listeners: {
                        valid: {
                            fn: function(a) {
                                ...
                                _this.slider.setValue(timeStep);
                            }
                        },
                        specialkey: {
                            fn: function(t, e) {
        ...
        _this.dateFieldYear = new DateFieldSpinnerDialog({
            fieldLabel: 'Year',
            width: 45,
        ...
        _this.dateFieldMonth = new DateFieldSpinnerDialog({
            fieldLabel: 'Month',
            width: 34,
            validator: function(value) {
        ...
        _this.dateFieldDay = new DateFieldSpinnerDialog({
            fieldLabel: 'Day',
            width: 34,
            validator: function(value) {
                return NumberExpr2digits.test(value);
        ...
        _this.dateFieldHour = new DateFieldSpinnerDialog({
            fieldLabel: 'Hour',
            width: 34,
            validator: function(value) {
                return NumberExpr2digits.test(value);
        ...
        _this.dateFieldMinute = new DateFieldSpinnerDialog({
            fieldLabel: 'Minute',
            width: 34,
            validator: function(value) {
                return NumberExpr2digits.test(value);
        ...
        _this.dateFieldSecond = new DateFieldSpinnerDialog({
            fieldLabel: 'Second',
            width: 34,
            validator: function(value) {
                return NumberExpr2digits.test(value);
        ...
        _this.setLayersDimension = function(dateString) {
            try {
                for (var i = 0; i < this.timeLayers.length; i++) {
                    this.timeLayers[i].mergeNewParams({
                        'time': dateString
                    });
...
function parseISO8601DateToDate(isotime) {
    try {
        ...
        date.toISO8601 = function() {
            ...
            var iso = prf(this.getUTCFullYear(), 4) + "-" + prf(this.getUTCMonth() + 1, 2) + "-" + prf(this.getUTCDate(), 2) + "T" + prf(this.getUTCHours(), 2) + ":" + prf(this.getUTCMinutes(), 2) + ":" + prf(this.getUTCSeconds(), 2) + 'Z';
            return iso;
        }
        date.clone = function() {
...
Heron.widgets.search.NominatimSearchCombo = Ext.extend(Ext.form.ComboBox, {
    ...
    storeFields: ["place_id", "display_name", {
        name: "address",
        type: "Object"
    }, "boundingbox", {
        name: "lat",
    ...
    initComponent: function() {
        ...
        if (this.extent) {
            ...
            maxExt.transform(this.map.getProjectionObject(), new OpenLayers.Projection("EPSG:4326"));
...
Heron.widgets.search.SearchByFeaturePanel = Ext.extend(Heron.widgets.search.SpatialSearchPanel, {
    ...
    initComponent: function() {
        ...
        this.items = [this.createSourceLayerCombo(), this.createDrawFieldSet(), this.createTargetLayerCombo({
            selectFirst: true
        }), this.createSearchTypeCombo(), this.createSearchDistField(), this.createActionButtons(), this.createStatusPanel(), this.resetButton];
...
Heron.widgets.search.SearchByBufferPanel = Ext.extend(Heron.widgets.search.SpatialSearchPanel, {
    ...
    initComponent: function() {
        ...
        this.items = [this.createSourceLayerCombo(), this.createVectorLayerCombo(), this.createSearchDistField(), this.bufferButton, this.createDrawFieldSet(), this.createTargetLayerCombo({
            selectFirst: true
        }), this.createActionButtons(), this.createStatusPanel(), this.resetButton];
    ...
    bufferFeatures: function(features) {
        ...
        for (var i = 0; i < features.length; i++) {
            ...
            buffer = input.buffer(distance * 1.25);
            buffer = parser.write(buffer);
            buffer = geoJSON.read(buffer, 'Geometry');
